  ...
  try {
    ...
    for (const userData of defaultUsers) {
      try {
        ...
        const errorMsg = `Failed to create user ${userData.email}: ${(userError as Error).message}`
...
export async function createDemoUser(email: string, password: string, role = "Admin") {
  try {
    // Check if user already exists
    const existingUser = await sql`
    ...
    return { success: true, message: "Demo user created successfully", user: newUser }
  } catch (error) {
