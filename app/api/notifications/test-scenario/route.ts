  ...
  try {
    ...
    const scenarios = {
      transaction_alert: {
        ...
        message:
          "A high-value transaction of GHS 50,000 has been processed on your account.",
        priority: "high",
      },
      low_balance: {
        subject: "Low Balance Warning",
        message:
          "Your MoMo float balance is below the minimum threshold (GHS 1,000).",
        priority: "medium",
      },
      security_alert: {
    ...
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to test scenario",
      },
      { status: 500 }
    );
  }
