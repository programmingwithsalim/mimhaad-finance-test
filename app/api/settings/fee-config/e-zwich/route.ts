...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      "[FEE-CONFIG] Loading E-Zwich fee config for:",
      transactionType
    );
    ...
    return NextResponse.json({
      ...
      config: {
        ...
        minimum_fee: feeConfig.minimum_fee
          ? Number(feeConfig.minimum_fee)
          : null,
        maximum_fee: feeConfig.maximum_fee
          ? Number(feeConfig.maximum_fee)
          : null,
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Missing required fields: transaction_type, fee_type, fee_value",
        },
        { status: 400 }
      );
    }
